function Sequence = plotSpikesFrequenciesDepths(subject, subject_rec, direc, imec, sp, blockEvents, sec_before_trigger, Depth1, Depth2, idx_chosen)
%
...
for i = 1: num_depths
    ...
    for m=1:2   
        ...
        if m==1
            s(1) = scatter(Sequence(m,i).frequencies,Sequence(m,i).post_trig,'bo', 'DisplayName', 'Random fifty');
        else
            s(2) = scatter(Sequence(m,i).frequencies,Sequence(m,i).post_trig,'ro', 'DisplayName', 'Random');
        end
    end
