function [cl_final, pre_trig, post_trig, p] = findSignificantClustersOmission(spTimes, spSites, cl_chosen, triggersInterest, sec_before_trigger)
%
% Finding good clusters as the ones with significant difference between
% before and after ONLY the omitted triggers (even ones).
%
% Inputs:
...
for i = 1:length(cl_chosen)
    ...
    % sec_before_trigger). ONLY FOR EVEN TRIGGERS (omitted in spaced
    % sequences) = 2:2.
    for k = 2:2:length(triggersInterest)
